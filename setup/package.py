...
class BetopiaERPTestTimeoutError(Exception):
    pass


class BetopiaERPTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise BetopiaERPTestError("Installation of package failed")
        else:
        ...
        raise BetopiaERPTestError("Package test: FAILED. Not able to install base.")
...
class Docker():
    ...
    def test_betopiaerp(self):
        ...
        if self.is_running():
            self.stop()
            raise BetopiaERPTestTimeoutError('BetopiaERP pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise BetopiaERPTestError('Error while installing/starting BetopiaERP after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))

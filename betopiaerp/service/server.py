...
from betopiaerp import api, sql_db
from betopiaerp.modules.registry import Registry
from betopiaerp.release import nt_service_name
from betopiaerp.tools import config, gc, osutil, OrderedSet, profiler
from betopiaerp.tools.cache import log_ormcache_stats
from betopiaerp.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    import betopiaerp  # for eventd
    if betopiaerp.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(CommonRequestHandler):
    def setup(self):
        ...
        me.name = 'betopiaerp.service.http.request.%s' % (me.ident,)
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in betopiaerp.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="betopiaerp.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from betopiaerp.addons.base.models.ir_cron import IrCron  # noqa: PLC0415
    ...
    def cron_spawn(self):
        ...
        for i in range(config['max_cron_threads']):
            t = threading.Thread(target=self.cron_thread, args=(i,), name=f"betopiaerp.service.cron.cron{i}")
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from betopiaerp.tests.result import _logger as logger  # noqa: PLC0415
...
def load_server_wide_modules():
    from betopiaerp.modules.module import load_openerp_module  # noqa: PLC0415
...
def start(preload=None, stop=False):
    ...
    import betopiaerp.http  # noqa: PLC0415

    if betopiaerp.evented:
        server = GeventServer(betopiaerp.http.root)
    elif config['workers']:
        if config['test_enable']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(betopiaerp.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which BetopiaERP is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(betopiaerp.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not betopiaerp.evented:
        if inotify:

...
import betopiaerp.api
import betopiaerp.modules.neutralize
import betopiaerp.release
import betopiaerp.sql_db
import betopiaerp.tools
from betopiaerp.exceptions import AccessDenied
from betopiaerp.release import version_info
from betopiaerp.sql_db import db_connect
from betopiaerp.tools import osutil, SQL
from betopiaerp.tools.misc import exec_pg_environ, find_pg_tool
...
def check_db_management_enabled(func, /):
    ...
    def if_db_mgt_enabled(*args, **kwargs):
        if not betopiaerp.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and betopiaerp.tools.config.verify_admin_password(passwd):
        return True
    raise betopiaerp.exceptions.AccessDenied()


# This should be moved to betopiaerp.modules.db, along side initialize().
def _initialize_db(db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        betopiaerp.tools.config['load_language'] = lang

        registry = betopiaerp.modules.registry.Registry.new(db_name, update_module=True, new_db_demo=demo)

        with closing(registry.cursor()) as cr:
            env = betopiaerp.api.Environment(cr, betopiaerp.api.SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = betopiaerp.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = betopiaerp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = betopiaerp.tools.config['db_template']
    ...
    try:
        db = betopiaerp.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if betopiaerp.tools.config['unaccent']:
                ...
                # But in the case of BetopiaERP, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    betopiaerp.sql_db.close_db(db_original_name)
    db = betopiaerp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = betopiaerp.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = betopiaerp.api.Environment(cr, betopiaerp.api.SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            betopiaerp.modules.neutralize.neutralize_database(cr)

    from_fs = betopiaerp.tools.config.filestore(db_original_name)
    to_fs = betopiaerp.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    betopiaerp.modules.registry.Registry.delete(db_name)
    betopiaerp.sql_db.close_db(db_name)
    ...
    fs = betopiaerp.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        ...
        'major_version': betopiaerp.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip', with_filestore=True):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            if with_filestore:
                filestore = betopiaerp.tools.config.filestore(db_name)
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = betopiaerp.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = betopiaerp.api.Environment(cr, betopiaerp.api.SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    betopiaerp.modules.registry.Registry.delete(old_name)
    betopiaerp.sql_db.close_db(old_name)
    ...
    old_fs = betopiaerp.tools.config.filestore(old_name)
    new_fs = betopiaerp.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    betopiaerp.tools.config.set_admin_password(new_password)
    betopiaerp.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        betopiaerp.modules.registry.Registry.new(db, update_module=True, upgrade_modules={'base'})
    return True
...
@betopiaerp.tools.mute_logger('betopiaerp.sql_db')
...
def list_dbs(force=False):
    if not betopiaerp.tools.config['list_db'] and not force:
        raise betopiaerp.exceptions.AccessDenied()

    if not betopiaerp.tools.config['dbfilter'] and betopiaerp.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, BetopiaERP will not
        ...
        return sorted(betopiaerp.tools.config['db_name'])
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of BetopiaERP
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if betopiaerp.tools.sql.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        betopiaerp.sql_db.close_db(database_name)
...
def exp_list_lang():
    return betopiaerp.tools.misc.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(betopiaerp.tools.config.root_path, 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return betopiaerp.release.version

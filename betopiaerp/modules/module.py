...
import betopiaerp.tools as tools
import betopiaerp.upgrade
...
class UpgradeHook:
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("betopiaerp.addons.base.maintenance.migrations", "betopiaerp.upgrade")
...
def initialize_sys_path() -> None:
    """
    Setup the addons path ``betopiaerp.addons.__path__`` with various defaults
    ...
    for path in (
        ...
        *tools.config['addons_path'],
        ...
        if os.access(path, os.R_OK) and path not in betopiaerp.addons.__path__:
            betopiaerp.addons.__path__.append(path)

    # hook betopiaerp.upgrade on upgrade-path
    ...
    for up in tools.config['upgrade_path'] or [legacy_upgrade_path]:
        if up not in betopiaerp.upgrade.__path__:
            betopiaerp.upgrade.__path__.append(up)

    # create decrecated module alias from betopiaerp.addons.base.maintenance.migrations to betopiaerp.upgrade
    spec = importlib.machinery.ModuleSpec("betopiaerp.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = betopiaerp.upgrade  # type: ignore
    sys.modules["betopiaerp.addons.base.maintenance"] = maintenance_pkg
    sys.modules["betopiaerp.addons.base.maintenance.migrations"] = betopiaerp.upgrade

    # hook for upgrades and namespace freeze
    if not getattr(initialize_sys_path, 'called', False):  # only initialize once
        betopiaerp.addons.__path__._path_finder = lambda *a: None  # prevent path invalidation
        betopiaerp.upgrade.__path__._path_finder = lambda *a: None  # prevent path invalidation
...
class Manifest(Mapping[str, typing.Any]):
    ...
    def _get_manifest_from_addons(module: str) -> Manifest | None:
        ...
        for adp in betopiaerp.addons.__path__:
...
def get_resource_from_path(path: str) -> tuple[str, str, str] | None:
    ...
    """
    resource = None
    sorted_paths = sorted(betopiaerp.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name: str) -> None:
    ...
    qualname = f'betopiaerp.addons.{module_name}'

...
""" Initialize the database for module management and BetopiaERP installation. """
...
if typing.TYPE_CHECKING:
    from betopiaerp.sql_db import BaseCursor, Cursor
...
def is_initialized(cr: Cursor) -> bool:
    ...
    """
    return betopiaerp.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr: Cursor) -> None:
    ...
    """
    try:
        f = betopiaerp.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise OSError(m)

    with betopiaerp.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for info in betopiaerp.modules.Manifest.all_addon_manifests():
    ...
    from betopiaerp.tools import config  # noqa: PLC0415

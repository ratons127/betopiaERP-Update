...
if typing.TYPE_CHECKING:
    ...
    from betopiaerp.orm.environments import Transaction
...
class Cursor(BaseCursor):
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # BetopiaERP Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def db_connect(to: str, allow_uri=False, readonly=False) -> Connection:
    ...
    maxconn = (tools.config['db_maxconn_gevent'] if hasattr(betopiaerp, 'evented') and betopiaerp.evented else 0) or tools.config['db_maxconn']
...
def close_db(db_name: str) -> None:
    """ You might want to call betopiaerp.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:

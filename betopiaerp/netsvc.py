...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "betopiaerp.netsvc.log is deprecated starting BetopiaERP 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('betopiaerp.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('betopiaerp.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='betopiaerp.tools.image')
...
DEFAULT_LOG_CONFIGURATION = [
    'betopiaerp.http.rpc.request:INFO',
    'betopiaerp.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {

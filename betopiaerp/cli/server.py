...
from betopiaerp.release import author as __author__  # noqa: F401
from betopiaerp.release import version as __version__  # noqa: F401
from betopiaerp.service import server
...
def report_configuration():
    ...
    import betopiaerp.release  # noqa: PLC0415
    _logger.info("BetopiaERP version %s", betopiaerp.release.version)
    if os.path.isfile(config['config']):
        _logger.info("Using configuration file at %s", config['config'])
    _logger.info('addons paths: %s', betopiaerp.addons.__path__)
    ...
    if sys.version_info[:2] > betopiaerp.release.MAX_PY_VERSION:
        _logger.warning("Python %s is not officially supported, please use Python %s instead",
            '.'.join(map(str, sys.version_info[:2])),
            '.'.join(map(str, betopiaerp.release.MAX_PY_VERSION))
        )
...
def setup_pid_file():
    ...
    This function assumes the configuration has been initialized.
    """
    import betopiaerp  # for evented  # noqa: PLC0415
    if not betopiaerp.evented and config['pidfile']:
        pid = os.getpid()
...
def main(args):
    ...
    for db_name in config['db_name']:
        from betopiaerp.service import db  # noqa: PLC0415
        try:

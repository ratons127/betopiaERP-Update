...
import betopiaerp.tools.config
...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = betopiaerp.tools.config.parser
        ...
        opt = betopiaerp.tools.config.parse_config(args, setup_logging=True)
        ...
        try:
            with betopiaerp.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = betopiaerp.modules.neutralize.get_installed_modules(cursor)
                    queries = betopiaerp.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    betopiaerp.modules.neutralize.neutralize_database(cursor)

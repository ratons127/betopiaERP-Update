...
from betopiaerp.modules import initialize_sys_path, load_script
...
def load_internal_commands():
    """ Load ``commands`` from ``betopiaerp.cli`` """
    for path in betopiaerp.cli.__path__:
        for module in Path(path).iterdir():
            if module.suffix != '.py':
                continue
            __import__(f'betopiaerp.cli.{module.stem}')
    ...
    for path in betopiaerp.addons.__path__:
        for fullpath in Path(path).glob(f'*/cli/{command}.py'):
            if (found_command := fullpath.stem) and Command.is_valid_name(found_command):
                # loading as betopiaerp.cli and not betopiaerp.addons.{module}.cli
                # so it doesn't load betopiaerp.addons.{module}.__init__
                mapping[f'betopiaerp.cli.{found_command}'] = fullpath 
...
def find_command(name: str) -> Command | None:
    ...
    # import from betopiaerp.cli
    with contextlib.suppress(ImportError):
        __import__(f'betopiaerp.cli.{name}')
        return commands[name]

    # import from betopiaerp.addons.*.cli
...
def main():
    ...
    if command := find_command(command_name):
        betopiaerp.cli.COMMAND = command_name
        command().run(args)
    else:
        message = (

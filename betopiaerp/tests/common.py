# -*- coding: utf-8 -*-
"""
The module :mod:`betopiaerp.tests.common` provides unittest test cases and a few
...
import betopiaerp.addons.base
import betopiaerp.cli
import betopiaerp.http
import betopiaerp.models
import betopiaerp.orm.registry
from betopiaerp import api
from betopiaerp.exceptions import AccessError
from betopiaerp.fields import Command
from betopiaerp.modules.registry import Registry, DummyRLock
from betopiaerp.service import security
from betopiaerp.sql_db import Cursor, Savepoint
from betopiaerp.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from betopiaerp.tools.mail import single_email_re
from betopiaerp.tools.misc import find_in_path, lower_logging
from betopiaerp.tools.xml_utils import _validate_xml
from betopiaerp.addons.base.models import ir_actions_report

from . import case, test_cursor
from .result import BetopiaERPTestResult
...
if betopiaerp.cli.COMMAND in ('server', 'start') and not config['test_enable']:
    _logger.error(
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: betopiaerp.tests.common.Form is deprecated, use betopiaerp.tests.Form",
...
def get_db_name():
    dbnames = betopiaerp.tools.config['db_name']
...
def release_test_lock():
    ...
    try:
        ...
        if not _registry_test_lock.acquire(timeout=60):
            tag = betopiaerp.modules.module.current_test.canonical_tag
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for BetopiaERP-specific code. This class is abstract and
    ...
    def __init_subclass__(cls):
        ...
        if cls.__module__.startswith('betopiaerp.addons.'):
    ...
    def run(self, result: BetopiaERPTestResult) -> None:
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~betopiaerp.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(betopiaerp.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            betopiaerp.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = betopiaerp.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('betopiaerp.sql_db.Cursor.execute', execute),
    ...
    def assertRecordValues(
            self,
            records: betopiaerp.models.BaseModel,
        ...
        for record in records:
            ...
            for field_name in field_names:
                ...
                match record._fields[field_name]:
                    case betopiaerp.fields.Many2one():
                        record_value = record_value.id
                    case betopiaerp.fields.One2many() | betopiaerp.fields.Many2many():
                        record_value = sorted(record_value.ids)
                    case betopiaerp.fields.Float() as field if digits := field.get_digits(record.env):
                        record_value = Approx(record_value, digits[1], decorate=False)
                    case betopiaerp.fields.Monetary() as field if currency_field_name := field.get_currency_field(record):
    ...
    def assertCanOpenTestCursor(self):
        ...
        if betopiaerp.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {betopiaerp.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = betopiaerp.http.request
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if betopiaerp.tools.config['test_tags'] and 'is_query_count' in betopiaerp.tools.config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | betopiaerp.addons.base.models.res_currency.ResCurrency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    """
    muted_registry_logger = mute_logger(betopiaerp.orm.registry._logger.name)
    freeze_time = None
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('betopiaerp.addons.base.models.res_users.ResUsersPatchedInTest._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(betopiaerp.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way BetopiaERP widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    touch_enabled = False
    session: betopiaerp.http.Session = None
    ...
    @classmethod
    def http_port(cls):
        if betopiaerp.service.server.server is None:
            return None
        return betopiaerp.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('betopiaerp.service.http.request.')]
        ...
        if request_threads:
            ...
            betopiaerp.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        betopiaerp.http.root.session_store.save(self.session)

    def authenticate(self, user, password, *,
        browser: ChromeBrowser = None, session_extra: dict | None = None):
        if getattr(self, 'session', None):
            betopiaerp.http.root.session_store.delete(self.session)

        self.session = session = betopiaerp.http.root.session_store.new()
        session.update(
            betopiaerp.http.get_default_session(),
            db=get_db_name(),
            # In order to avoid perform a query to each first `url_open`
            # in a test (insert `res.device.log`).
            _trace_disable=True,
        )
        session.context['lang'] = betopiaerp.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('betopiaerp.addons.base.models.res_users.ResUsersPatchedInTest._check_credentials', new=patched_check_credentials):
        ...
        betopiaerp.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        with self.allow_requests(browser=browser), contextlib.ExitStack() as atexit:
            ...
            if "bus.bus" in self.env.registry:
                from betopiaerp.addons.bus.websocket import CloseCode, _kick_all, WebsocketConnectionHandler  # noqa: PLC0415
                from betopiaerp.addons.bus.models.bus import BusBus  # noqa: PLC0415
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('betopiaerp.http.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if betopiaerp.tools.config['test_tags'] and 'is_tour' in betopiaerp.tools.config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from betopiaerp.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in BetopiaERP test suites

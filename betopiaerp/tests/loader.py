...
from .suite import BetopiaERPSuite
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'betopiaerp.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"betopiaerp.upgrade.{module}",
        f"betopiaerp.addons.{module}.migrations",
        f"betopiaerp.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return BetopiaERPSuite(sorted(tests, key=lambda t: getattr(t, 'test_sequence', 0)))
...
def run_suite(suite, global_report=None):
    ...
    results = BetopiaERPTestResult(global_report=global_report)
    suite(results)

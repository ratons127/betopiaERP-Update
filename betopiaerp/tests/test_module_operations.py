...
from betopiaerp.tools import config, topological_sort, unique, profiler
from betopiaerp.modules.registry import Registry
from betopiaerp.netsvc import init_logger
from betopiaerp.tests import standalone_tests
import betopiaerp.tests.loader

_logger = logging.getLogger('betopiaerp.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of BetopiaERP modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=addons_path,
        help="Comma-separated list of paths to directories containing extra BetopiaERP modules")
...
def test_standalone(args):
    ...
    betopiaerp.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        betopiaerp.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    if args.addons_path:
        ...
        betopiaerp.modules.module.initialize_sys_path()
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'betopiaerp.modules.loading': {'level': 'CRITICAL'},
            'betopiaerp.sql_db': {'level': 'CRITICAL'},
            'betopiaerp.models.unlink': {'level': 'WARNING'},
            'betopiaerp.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })

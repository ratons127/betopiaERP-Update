...
def run_tests(env, test_tags, modules=None, reload_tests=False):
    ...
    from betopiaerp.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    betopiaerp.tools.config['test_tags'] = test_tags
    betopiaerp.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    betopiaerp.tools.config['test_enable'] = None
    betopiaerp.tools.config['test_tags'] = None

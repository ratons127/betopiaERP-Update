...
from betopiaerp.tools.func import locked, reset_cached_properties
from betopiaerp.tools.lru import LRU
from betopiaerp.tools.misc import Collector, format_frame
...
if typing.TYPE_CHECKING:
    ...
    from betopiaerp.fields import Field
    from betopiaerp.models import BaseModel
    from betopiaerp.sql_db import BaseCursor, Connection, Cursor
    from betopiaerp.modules import module_graph


_logger = logging.getLogger('betopiaerp.registry')
_schema = logging.getLogger('betopiaerp.schema')
...
class Registry(Mapping[str, type["BaseModel"]]):
        ...
        *,
        ...
        try:
            ...
            with registry.cursor() as cr:
                ...
                from betopiaerp.modules import db  # noqa: PLC0415
                if db.is_initialized(cr):
                    cr.execute("DELETE FROM ir_config_parameter WHERE key='base.partially_updated_database'")
                    if cr.rowcount:
                        update_module = True
            # This should be a method on Registry
            from betopiaerp.modules.loading import load_modules, reset_modules_state  # noqa: PLC0415
    ...
    def init(self, db_name: str) -> None:
        ...
        if config['test_enable']:
            from betopiaerp.tests.result import BetopiaERPTestResult  # noqa: PLC0415
            self._assertion_report: BetopiaERPTestResult | None = BetopiaERPTestResult()
        else:
        ...
        for indexname, tablename, field in expected:
            ...
            if index and indexname not in existing:
                ...
                if index == 'trigram':
                    ...
                    from betopiaerp.modules.db import FunctionStatus  # noqa: PLC0415

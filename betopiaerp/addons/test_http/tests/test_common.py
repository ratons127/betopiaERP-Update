...
import betopiaerp
from betopiaerp.http import Session
from betopiaerp.addons.base.tests.common import HttpCaseWithUserDemo
from betopiaerp.tools import config, reset_cached_properties
from betopiaerp.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(betopiaerp.http.Application, 'session_store', session_store)
        cls.classPatch(betopiaerp.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(betopiaerp.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        betopiaerp.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('betopiaerp.http.db_list') as db_list, \
             patch('betopiaerp.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('betopiaerp.http.db_list') as db_list, \
             patch('betopiaerp.http.db_filter') as db_filter, \
             patch('betopiaerp.http.Registry') as Registry:

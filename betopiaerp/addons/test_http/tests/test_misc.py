...
import betopiaerp
from betopiaerp.http import content_disposition, root
from betopiaerp.tests import tagged
from betopiaerp.tests.common import HOST, BaseCase, get_db_name, new_test_user
from betopiaerp.tools import config, file_path, mute_logger, parse_version

from .test_common import TestHttpBase
from betopiaerp.addons import test_http
from betopiaerp.addons.test_http.controllers import CT_JSON
from betopiaerp.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = betopiaerp.tools.misc.DotDict(db=False)
            return betopiaerp.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'betopiaerp.com/{uri}', host='betopiaerp.com'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://betopiaerp.com/{uri}', host='betopiaerp.com'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'betopiaerp.com/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://betopiaerp.com/{uri}'), "No host allowed")
    ...
    def test_misc4_rpc_qweb(self):
        ...
        for method in (self.db_url_open, self.nodb_url_open):
            with self.subTest(method=method.__name__):
                with mute_logger('betopiaerp.addons.rpc.controllers.jsonrpc'):
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'betopiaerp.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict(betopiaerp.tools.config.options, {'proxy_mode': True}):
    ...
    def test_misc8_concurrency_error(self):
        with (
            self.assertLogs('betopiaerp.service.model') as log_catcher,
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(betopiaerp.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = betopiaerp.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            betopiaerp.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')

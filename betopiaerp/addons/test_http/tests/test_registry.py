...
from betopiaerp.sql_db import close_db, db_connect
from betopiaerp.tests import HOST, BaseCase, Like, get_db_name, tagged
from betopiaerp.tools import mute_logger, reset_cached_properties, SQL
from betopiaerp.tools.urls import urljoin
...
class TestHttpRegistry(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.addClassCleanup(reset_cached_properties, betopiaerp.http.root)
        cls.classPatch(betopiaerp.conf, 'server_wide_modules', ['base', 'web', 'rpc', 'test_http'])

        # make sure there are always many databases, to break monodb
        cls._db_list = cls.startClassPatcher(patch('betopiaerp.http.db_list'))
        cls._db_list.return_value = ['postgres', get_db_name()]
        cls.startClassPatcher(patch('betopiaerp.http.db_filter',
    ...
    def authenticate(self, *, db=None):
        session = betopiaerp.http.root.session_store.new()
        session.update(betopiaerp.http.get_default_session(), db=db or get_db_name())
    ...
    def url_open(self, path, *, allow_redirects=False):
        ...
        url = urljoin(f"http://{HOST}:{betopiaerp.tools.config['http_port']}", path)
    ...
    def test_signaling(self):
        ...
        with self.assertLogs('betopiaerp.registry', logging.INFO) as capture:
            res = self.url_open('/test_http/ensure_db')
            self.assertEqual(res.status_code, 200)
        self.assertEqual(capture.output, [
            "INFO:betopiaerp.registry:Reloading the model registry after database signaling.",
            Like("INFO:betopiaerp.registry:Registry loaded in ...s"),
        ])
    ...
    def test_missing_db(self):
        ...
        with self.assertLogs('betopiaerp.http', logging.WARNING) as capture:
            res = self.url_open('/test_http/ensure_db')
        ...
        self.assertEqual(capture.output, [
            Like("WARNING:betopiaerp.http:Database or registry unusable, trying without\n"
                 f'Traceback...database "{db_duplicate}" does not exist...')
        ] * 2)

    @mute_logger('betopiaerp.sql_db')
    def test_corrupt_ir_module_module_table(self):
        ...
        with self.assertLogs('betopiaerp.registry', logging.ERROR) as capture1, \
             self.assertLogs('betopiaerp.http', logging.WARNING) as capture2:
            res = self.url_open('/test_http/greeting-public')
            self.assertEqual(res.status_code, 404)
        self.assertEqual(capture1.output, [
            "ERROR:betopiaerp.registry:Failed to load registry",
        ])

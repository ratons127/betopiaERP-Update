...
from betopiaerp.tests import get_db_name, tagged
from betopiaerp.tools import config, mute_logger, reset_cached_properties
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('betopiaerp.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session01_default_session(self):
        ...
        with patch.object(betopiaerp.http.root.session_store, 'save') as mock_save:
    ...
    def test_session03_logout_15_0_geoip(self):
        ...
        with self.assertLogs('betopiaerp.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:betopiaerp.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    def test_session09_logout(self):
        ...
        self.assertTrue(betopiaerp.http.root.session_store.get(sid), "session should exist")
        self.url_open('/web/session/logout', allow_redirects=False).raise_for_status()
        self.assertFalse(betopiaerp.http.root.session_store.get(sid), "session should not exist")
    ...
    def test_session10_explicit_session(self):
        ...
        with self.assertLogs('betopiaerp.http') as capture:
    ...
    def test_session12_x_betopiaerp_database_good_no_prior_session(self):
        res = self.multidb_url_open(
            '/test_http/ensure_db',
            dblist=('a', 'b'),
            headers={'X-BetopiaERP-Database': 'a'})
    ...
    def test_session15_x_betopiaerp_database_with_prior_session_different(self):
        ...
        res = self.url_open('/test_http/ensure_db', headers={
            'X-BetopiaERP-Database': f'not-{get_db_name()}',
        })
    ...
    def test_session16_soft_rotate_with_abort(self):
        ...
        # the abort in betopiaerp.http.JsonRPCDispatcher.dispatch.
...
class TestSessionStore(HttpCaseWithUserDemo):
    def setUp(self):
        ...
        reset_cached_properties(betopiaerp.http.root)
        self.addCleanup(reset_cached_properties, betopiaerp.http.root)
        patcher = patch.dict(config.options, {'data_dir': self.tmpdir.name})
        self.startPatcher(patcher)

    @mute_logger('betopiaerp.http')
    def test01_session_nan(self):
        ...
        with self.assertLogs('betopiaerp.http', level='WARNING') as logs:
            self.assertEqual(betopiaerp.http.get_session_max_inactivity(self.env), SESSION_LIFETIME)
            self.assertEqual(logs.output[0], "WARNING:betopiaerp.http:Invalid value for 'sessions.max_inactivity_seconds', using default value.")
    ...
    def test02_session_lifetime_1week(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = betopiaerp.http.root.session_store.get(session.sid)
    ...
    def test04_session_lifetime_nodb(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = betopiaerp.http.root.session_store.get(session)

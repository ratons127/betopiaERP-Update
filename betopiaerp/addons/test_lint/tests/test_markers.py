...
class TestConflictMarkers(lint_case.LintCase):
    ...
    def test_conflict_markers(self):
        """ Test that there are no conflict markers left in BetopiaERP files """
        import betopiaerp.addons  # noqa: PLC0415

        counter = 0

        paths = sorted(os.path.abspath(p) for p in [*betopiaerp.addons.__path__, *betopiaerp.__path__])

...
from betopiaerp import api, fields, models, tools, _
from betopiaerp.api import SUPERUSER_ID
from betopiaerp.exceptions import AccessDenied, AccessError, UserError, ValidationError
from betopiaerp.fields import Command, Domain
from betopiaerp.http import request, DEFAULT_LANG
from betopiaerp.tools import email_domain_extract, is_html_empty, frozendict, reset_cached_properties, SQL
...
class ResUsers(models.Model):
    ...
    def _check_credentials(self, credential, env):
        ...
        * catch :class:`~betopiaerp.exceptions.AccessDenied` and perform their
          own checking
        * (re)raise :class:`~betopiaerp.exceptions.AccessDenied` if the
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by BetopiaERP (updates, module installation, ...)'))
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: betopiaerp.exceptions.AccessDenied when old password is wrong
        :raise: betopiaerp.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this BetopiaERP portal anymore.

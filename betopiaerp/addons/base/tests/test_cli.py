...
from betopiaerp.cli.command import commands, load_addons_commands, load_internal_commands
from betopiaerp.tests import BaseCase
from betopiaerp.tools import config, file_path
...
class TestCommand(BaseCase):
    ...
    def test_docstring(self):
        ...
        for name, cmd in commands.items():
            self.assertTrue(cmd.__doc__,
                msg=f"Command {name} needs a docstring to be displayed with 'betopiaerp-bin help'")
            self.assertFalse('\n' in cmd.__doc__ or len(cmd.__doc__) > 120,
                msg=f"Command {name}'s docstring format is invalid for 'betopiaerp-bin help'")
        ...
        for name in ('bonbon', 'café'):
            with self.subTest(name):
                ...
                self.assertEqual(
                    ...
                    "Use 'betopiaerp-bin --help' to see the list of available commands."
                )
    ...
    def test_upgrade_code_standalone(self):
        from betopiaerp.cli import upgrade_code  # noqa: PLC0415
        proc = sp.run(

...
from betopiaerp.tests.common import BaseCase
from betopiaerp.modules.module_graph import ModuleGraph
...
class TestGraph(BaseCase):
    @mute_logger('betopiaerp.modules.module_graph')
    def _test_graph_order(
        ...
        with (
            patch('betopiaerp.modules.module_graph.ModuleGraph._update_from_database'),
            patch('betopiaerp.modules.module_graph.Manifest.for_addon', make_manifest),
            patch('betopiaerp.modules.module_graph.ModuleGraph._imported_modules', {'studio_customization'}),
        ):
            dummy_cr = None

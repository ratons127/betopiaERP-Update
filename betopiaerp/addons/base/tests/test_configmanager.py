...
import betopiaerp
from betopiaerp.tests import TransactionCase
from betopiaerp.tools import file_open, file_open_temporary_directory, file_path
...
WARNING:betopiaerp.tools.config:missing --http-interface/http_interface, \
...
class TestConfigManager(TransactionCase):
    ...
    def test_01_default_config(self):
        self.assertConfigEqual({
            ...
            'proxy_access_token': '',
            'publisher_warranty_url': 'http://services.betopiaerp.com/publisher-warranty/',
            'reportgz': False,
    ...
    def test_02_config_file(self):
        ...
        with self.assertLogs('betopiaerp.tools.config', 'WARNING') as capture:
    ...
    def test_04_betopiaerp16_config_file(self):
        ...
        with (
            ...
            self.assertLogs('betopiaerp.tools.config', 'WARNING') as capture_warn,
        ):
        ...
        def missing(*options):
            return [
                f"WARNING:betopiaerp.tools.config:unknown option '{option}' in "
        ...
        def falsy(*options):
            return [
                f"WARNING:betopiaerp.tools.config:option {option} reads 'False' "
    ...
    def test_06_cli(self):
        ...
        self.assertEqual(capture.output, [
            "WARNING:betopiaerp.tools.config:test file '/tmp/file-file' cannot be found",
        ])
        ...
        self.assertConfigEqual({
            ...
            'log_handler': [
                ':WARNING',
                'betopiaerp.tools.config:DEBUG',
                'betopiaerp.http:DEBUG',
                'betopiaerp.sql_db:DEBUG',
            ],
            'log_db': 'logdb',
            'log_db_level': 'debug',
            'log_level': 'debug',
        ...
        for args, stop_after_init in [
            ...
            with self.subTest(args=args):
                if any('--test' in arg for arg in args):
                    ...
                    self.assertEqual(capture.output, [
                        "WARNING:betopiaerp.tools.config:Empty -d/--database/db_name, tests won't run",
                    ])
                else:
                    _, options = self.parse_reset(args)

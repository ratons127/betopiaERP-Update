...
from betopiaerp.tools.misc import OrderedSet
...
def url_to_module_path(url):
    """
    BetopiaERP modules each have a name. (betopiaerp.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_betopiaerp_define(module_path, dependencies, content):
    ...
    """
    return f"""betopiaerp.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
def get_aliased_betopiaerp_define_content(module_path, content):
    ...
    > betopiaerp.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\nbetopiaerp.define(`%s`, ['%s'], function (require) {
                        return require('%s');

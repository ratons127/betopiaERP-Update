...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in betopiaerp.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_path = {
            ...
            if (m := betopiaerp.modules.Manifest.for_addon(name, display_warning=False))
        }
        ...
        for module_name in module_list:
            manifest = betopiaerp.modules.Manifest.for_addon(module_name)
    ...
    def count_customization(self, env):
        ...
        # the module should be betopiaerp/studio unless it comes from an imported module install
        ...
        for f in env['ir.model.fields'].browse(data.keys()):
            self.book(
                data[f.id][0] or "betopiaerp/studio",
    ...
    def count_database(self, database):
        registry = betopiaerp.modules.registry.Registry(database)
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="BetopiaERP cloc", lines="Line", other="Other", code="Code")
        s += hr

...
from betopiaerp import release
from betopiaerp.tools.func import classproperty
from . import appdirs
...
class _BetopiaERPOption(optparse.Option):
...
class _FileOnlyOption(_BetopiaERPOption):
...
class _PosixOnlyOption(_BetopiaERPOption):
    def __init__(self, *opts, **attrs):
        if os.name != 'posix':
...
class configmanager:
    def __init__(self):
        ...
        # dictionary mapping option destination (keys in self.options) to BetopiaERPOptions.
        self.options_index = {}
    ...
    @property
    def rcfile(self):
        self._warn("Since 19.0, use betopiaerp.tools.config['config'] instead", DeprecationWarning, stacklevel=2)
        return self['config']

    @rcfile.setter
    def rcfile(self, rcfile):
        self._warn(f"Since 19.0, use betopiaerp.tools.config['config'] = {rcfile!r} instead", DeprecationWarning, stacklevel=2)
        self._runtime_options['config'] = rcfile

    def _build_cli(self):
        BetopiaERPOption = type('BetopiaERPOption', (_BetopiaERPOption,), {'config': self})
        FileOnlyOption = type('FileOnlyOption', (_FileOnlyOption, BetopiaERPOption), {})
        PosixOnlyOption = type('PosixOnlyOption', (_PosixOnlyOption, BetopiaERPOption), {})

        version = "%s %s" % (release.description, release.version)
        parser = optparse.OptionParser(version=version, option_class=BetopiaERPOption)
        ...
        parser.add_option(FileOnlyOption(dest='publisher_warranty_url', my_default='http://services.betopiaerp.com/publisher-warranty/', file_exportable=False))
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", type='path',  # sensitive default set in _load_default_options
                         help="Directory where to store BetopiaERP data")
        ...
        group.add_option('--log-handler', action="append", type='comma', my_default=[':INFO'], metavar="MODULE:LEVEL",
                         help='setup a handler at LEVEL for a given MODULE. An empty MODULE indicates the root logger. '
                              'This option can be repeated. Example: "betopiaerp.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const=("betopiaerp.http:DEBUG",),
                         help='shortcut for --log-handler=betopiaerp.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const=("betopiaerp.sql_db:DEBUG",),
                         help='shortcut for --log-handler=betopiaerp.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate BetopiaERP to another language. "
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes betopiaerp.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            betopiaerp.tools.config.parse_config(sys.argv[1:])
        """
        from betopiaerp import modules, netsvc  # noqa: PLC0415
        ...
        if setup_logging is not False:
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of BetopiaERP 18, it's recommended to specify whether"
                    " you want BetopiaERP to setup its own logging (or want to"
                    " handle it yourself)",

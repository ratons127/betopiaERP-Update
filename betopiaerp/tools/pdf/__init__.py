...
from betopiaerp import modules
from betopiaerp.tools.arabic_reshaper import reshape
from betopiaerp.tools.parse_version import parse_version
from betopiaerp.tools.misc import file_open, SENTINEL
...
def add_banner(pdf_stream, text=None, logo=False, thickness=SENTINEL):
    """ Add a banner on a PDF in the upper right corner, with BetopiaERP's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display BetopiaERP's logo in the banner.
...
class BetopiaERPPdfFileReader(PdfFileReader):
...
class BetopiaERPPdfFileWriter(PdfFileWriter):

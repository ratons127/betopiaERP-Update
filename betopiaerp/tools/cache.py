...
def log_ormcache_stats(sig=None, frame=None):    # noqa: ARG001 (arguments are there for signals)
    ...
    if sig == signal.SIGUSR1:
        threading.Thread(target=_log_ormcache_stats,
                         name="betopiaerp.signal.log_ormcache_stats").start()
    elif sig == signal.SIGUSR2:
        show_size = True
        threading.Thread(target=_log_ormcache_stats,
                         name="betopiaerp.signal.log_ormcache_stats_with_size").start()
...
def get_cache_size(
    ...
    from betopiaerp.api import Environment  # noqa: PLC0415

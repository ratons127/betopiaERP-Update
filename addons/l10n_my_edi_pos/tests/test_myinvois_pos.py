...
from betopiaerp import fields
from betopiaerp.exceptions import UserError, ValidationError
from betopiaerp.tests import tagged
from betopiaerp.tools import file_open, mute_logger

from betopiaerp.addons.account.tests.common import AccountTestInvoicingCommon
from betopiaerp.addons.l10n_my_edi.tests.test_file_generation import NS_MAP
from betopiaerp.addons.point_of_sale.tests.common import TestPoSCommon

CONTACT_PROXY_METHOD = 'betopiaerp.addons.l10n_my_edi.models.account_edi_proxy_user.AccountEdiProxyClientUser._l10n_my_edi_contact_proxy'
...
class TestMyInvoisPoS(TestPoSCommon):
    ...
    @mute_logger('betopiaerp.addons.point_of_sale.models.pos_order')
    ...
    def test_consolidate_invoices_limit(self):
        ...
        with freeze_time("2025-01-01"):
            ...
            with patch('betopiaerp.addons.l10n_my_edi_pos.wizard.myinvois_consolidate_invoice_wizard.MAX_LINE_COUNT_PER_INVOICE', 1):
                # Consolidate them
                wizard = self.env['myinvois.consolidate.invoice.wizard'].create({
                    'date_from': '2025-01-01',
                    'date_to': '2025-01-31',
    ...
    @mute_logger('betopiaerp.addons.point_of_sale.models.pos_order', 'betopiaerp.addons.point_of_sale.models.pos_session')
    ...
    def test_consolidate_invoices_refund_with_customer(self):
        """
        When an order has a customer set, BetopiaERP enforces that the refund must use the same customer.
        In the case of consolidated invoices, this does not make sense. So while we let BetopiaERP set the correct customer,

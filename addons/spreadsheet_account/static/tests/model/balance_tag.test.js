...
test("Basic evaluation", async () => {
    ...
    setCellContent(model, "A1", `=BETOPIAERP.BALANCE.TAG("10, 14", 2025)`);
...
test("with wrong date format", async () => {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=BETOPIAERP.BALANCE.TAG("10, 14", "This is not a valid date")`);
    await waitForDataLoaded(model);
...
test("with no date", async () => {
    ...
    setCellContent(model, "A1", `=BETOPIAERP.BALANCE.TAG("10, 14")`);
    await waitForDataLoaded(model);
    expect.verifySteps(["spreadsheet_fetch_balance_tag"]);
    expect(getCellValue(model, "A1")).toBe(100.0);
});

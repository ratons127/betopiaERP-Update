...
# betopiaerp-mailgate
#
# This program will read an email from stdin and forward it to betopiaerp. Configure
# a pipe alias in your mail server to use it, postfix uses a syntax that looks
# like:
#
# email@address: "|/home/betopiaerp/src/betopiaerp-mail.py"
#
# while exim uses a syntax that looks like:
#
# *: |/home/betopiaerp/src/betopiaerp-mail.py
...
def main():
    ...
    op.add_option("-d", "--database", dest="database", help="BetopiaERP database name (default: %default)", default='betopiaerp')
    op.add_option("-u", "--userid", dest="userid", help="BetopiaERP user id to connect with (default: %default)", default=1, type=int)
    op.add_option("-p", "--password", dest="password", help="BetopiaERP user password (default: %default)", default='admin')
    op.add_option("--host", dest="host", help="BetopiaERP host (default: %default)", default='localhost')
    op.add_option("--port", dest="port", help="BetopiaERP port (default: %default)", default=8069, type=int)
    ...
    try:
        ...
        if e.faultString == 'Access denied' and e.faultCode == 3:
            ...
            postfix_exit(EX_NOUSER, "alias does not exist in betopiaerp\n", o.debug)
        else:
...
try:
    ...
    # may include the password for the betopiaerp user.

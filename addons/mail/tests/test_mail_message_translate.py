...
class TestTranslationController(HttpCaseWithUserDemo):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="betopiaerp.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="betopiaerp.exceptions.AccessError"), mute_logger("betopiaerp.http"):
            self._mock_translation_request({"message_id": self.message.id})

...
from betopiaerp.tools import exception_to_unicode
...
class BetopiaERPIMAP4_SSL(BetopiaERPIMAP4, IMAP4_SSL):
    pass


class BetopiaERPPOP3(POP3):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._unread_messages = None
...
class BetopiaERPPOP3_SSL(BetopiaERPPOP3, POP3_SSL):
    pass
...
betopiaerp_mailgate: "|/path/to/betopiaerp-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
    ...
    def _connect__(self, allow_archived=False):  # noqa: PLW3201
        ...
        if connection_type == 'imap':
            ...
            connection = BetopiaERPIMAP4_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else BetopiaERPIMAP4(server, port, timeout=MAIL_TIMEOUT)
            self._imap_login__(connection)
        elif connection_type == 'pop':
            server, port, is_ssl = self.server, int(self.port), self.is_ssl
            connection = BetopiaERPPOP3_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else BetopiaERPPOP3(server, port, timeout=MAIL_TIMEOUT)

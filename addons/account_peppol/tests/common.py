...
class PeppolConnectorCommon(AccountTestInvoicingCommon):
    ...
    def _mock_can_connect(self, with_auth=False):
        ...
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/2/can_connect',
            replacement_method,
        )
    ...
    def _mock_connect(self, success=True, peppol_state='smp_registration', id_client='test_id_client'):
        def replacement_method(url, **kwargs):
            if peppol_state == 'rejected':
                return {
                    'status_code': 403,
                    'code': 201,
                    'message': 'Unable to register, please contact our support team at peppol.support@betopiaerp.com.'
                }
            if success:
        ...
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/2/connect',
            replacement_method,
        )

    def _mock_register_sender(self, success=True):
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/1/register_sender',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )

    def _mock_register_sender_as_receiver(self, success=True):
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/1/register_sender_as_receiver',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )
    ...
    def _mock_lookup_participant(self, already_exists=False):
        ...
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/1/lookup',
            replacement_method,
        )
    ...
    def _mock_participant_status(self, peppol_state, exists=True):
                    ...
                    'result': {
                        'error': {
                            ...
                            'message': "Your registration for this service is no longer valid. "
                                       "If you see this message, please update the related BetopiaERP app. "
        ...
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/2/participant_status',
            replacement_method,
        )

    def _mock_cancel_peppol_registration(self, success=True):
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/1/cancel_peppol_registration',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )

    def _mock_get_all_documents(self, success=True):
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/1/get_all_documents',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )

    def _mock_update_user(self, success=True):
        return (
            'https://peppol.test.betopiaerp.com/api/peppol/1/update_user',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )

...
from betopiaerp.addons.account.models.company import PEPPOL_LIST
...
class ResPartner(models.Model):
    ...
    peppol_eas = fields.Selection(selection_add=[('odemo', 'BetopiaERP Demo ID')])  # Not a real EAS, used for demonstration.
    ...
    def _check_peppol_participant_exists(self, participant_info, edi_identification):
        ...
        if isinstance(participant_info, dict):
            ...
            # DEPRECATED: we now use BetopiaERP peppol API to fetch participant info and get a json response
    ...
    def _peppol_lookup_participant(self, edi_identification):
        """NAPTR DNS peppol participant lookup through BetopiaERP's Peppol proxy"""

...
export class BetopiaERPChartCoreViewPlugin extends BetopiaERPUIPlugin {
    static getters = /** @type {const} */ (["getChartDataSource", "getBetopiaERPEnv"]);
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getBetopiaERPChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getBetopiaERPChartIds().includes(cmd.chartId)) {
                        continue;
                    }
                    if (this._shouldReloadDataSource(cmd.chartId, cmd.definition)) {
                        this._resetChartDataSource(cmd.chartId);
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getBetopiaERPChartDataSourceId(chartId);
    ...
    _getBetopiaERPChartDataSourceId(chartId) {
     ...
     */
    _refreshBetopiaERPChart(chartId) {
     ...
     */
    _refreshBetopiaERPCharts() {
        for (const chartId of this.getters.getBetopiaERPChartIds()) {
            this._refreshBetopiaERPChart(chartId);
        }
    }

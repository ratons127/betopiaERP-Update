...
export class BetopiaERPChartFeaturePlugin extends BetopiaERPUIPlugin {
    ...
    getAvailableChartGranularities(chartId) {
        ...
        const matching = this.getters.getBetopiaERPChartFieldMatching(chartId, filterId);
    ...
    _getChartHorizontalAxisFilter(chartId, fieldName) {
        for (const filter of this.getters.getGlobalFilters()) {
            const matching = this.getters.getBetopiaERPChartFieldMatching(chartId, filter.id);

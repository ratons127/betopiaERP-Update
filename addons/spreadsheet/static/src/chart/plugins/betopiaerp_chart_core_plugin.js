...
const CHART_PLACEHOLDER_DISPLAY_NAME = {
    betopiaerp_bar: _t("BetopiaERP Bar Chart"),
    betopiaerp_line: _t("BetopiaERP Line Chart"),
    betopiaerp_pie: _t("BetopiaERP Pie Chart"),
    betopiaerp_radar: _t("BetopiaERP Radar Chart"),
};

export class BetopiaERPChartCorePlugin extends BetopiaERPCorePlugin {
    static getters = /** @type {const} */ ([
        "getBetopiaERPChartIds",
        "getChartFieldMatch",
        "getBetopiaERPChartDisplayName",
        "getBetopiaERPChartFieldMatching",
        "getChartGranularity",
    ]);
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "CREATE_CHART": {
                if (cmd.definition.type.startsWith("betopiaerp_")) {
                    this._addBetopiaERPChart(cmd.chartId);
                }
                break;
            }
            case "DELETE_CHART": {
            ...
            case "EDIT_GLOBAL_FILTER":
                if (cmd.chart) {
                    this._setBetopiaERPChartFieldMatching(cmd.filter.id, cmd.chart);
                }
                break;
        }
    }
    ...
    /**
     * Get all the betopiaerp chart ids
        ...
        for (const sheet of data.sheets) {
            if (sheet.figures) {
                for (const figure of sheet.figures) {
                    if (figure.tag === "chart" && figure.data.type.startsWith("betopiaerp_")) {
                        this._addBetopiaERPChart(figure.data.chartId, figure.data.fieldMatching ?? {});
                        ...
                        for (const chartId in figure.data.chartDefinitions) {
                            ...
                            if (figure.data.chartDefinitions[chartId].type.startsWith("betopiaerp_")) {
                                this._addBetopiaERPChart(chartId, fieldMatching[chartId]);
                            }
                        }
                    }
                }
                ...
                for (const figure of sheet.figures) {
                    if (figure.tag === "chart" && figure.data.type.startsWith("betopiaerp_")) {
                        ...
                        for (const chartId in figure.data.chartDefinitions) {
                            ...
                            if (chartDefinition.type.startsWith("betopiaerp_")) {
    ...
    /**
     * Get the current betopiaerpChartFieldMatching of a chart
     *
     * @param {string} chartId
     * @param {string} filterId
     */
    getBetopiaERPChartFieldMatching(chartId, filterId) {
        return this.charts[chartId].fieldMatching[filterId];
    }

    /**
     * Sets the current betopiaerpChartFieldMatching of a chart
     *
     * @param {string} filterId
     * @param {Record<string,FieldMatching>} chartFieldMatches
     */
    _setBetopiaERPChartFieldMatching(filterId, chartFieldMatches) {
        const charts = { ...this.charts };
     ...
     */
    _addBetopiaERPChart(chartId, fieldMatching = undefined) {

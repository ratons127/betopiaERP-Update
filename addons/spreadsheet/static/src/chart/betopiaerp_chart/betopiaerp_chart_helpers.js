...
export function onBetopiaERPChartItemClick(getters, chart) {
    return navigateInBetopiaERPMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onWaterfallBetopiaERPChartItemClick(getters, chart) {
    return navigateInBetopiaERPMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onGeoBetopiaERPChartItemClick(getters, chart) {
    return navigateInBetopiaERPMenuOnClick(getters, chart, (chartJsItem) => {
...
export function onSunburstBetopiaERPChartItemClick(getters, chart) {
    return navigateInBetopiaERPMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
...
export function onTreemapBetopiaERPChartItemClick(getters, chart) {
    return navigateInBetopiaERPMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
        const rawItem = chartJSChart.data.datasets[datasetIndex].data[index];
...
function navigateInBetopiaERPMenuOnClick(getters, chart, getDomainFromChartItem) {
    return async (event, items, chartJSChart) => {
        const env = getters.getBetopiaERPEnv();
        const { datasets, labels } = chart.dataSource.getData();
...
export function onBetopiaERPChartItemHover() {
    return (event, items) => {
        if (items.length > 0) {
...
export function onGeoBetopiaERPChartItemHover() {
    return (event, items) => {
        if (!items.length) {
            event.native.target.style.cursor = "";
            return;
...
export async function navigateToBetopiaERPMenu(menu, actionService, notificationService, newWindow) {
    if (!menu) {

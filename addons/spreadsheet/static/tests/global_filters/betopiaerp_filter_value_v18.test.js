...
test("BETOPIAERP.FILTER.VALUE.V18 text filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=BETOPIAERP.FILTER.VALUE.V18("Text Filter")`);
...
test("BETOPIAERP.FILTER.VALUE.V18 empty date filter does't spill", async function () {
    mockDate("2022-03-10 00:00:00");
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=BETOPIAERP.FILTER.VALUE.V18("Date Filter")`);
...
test("BETOPIAERP.FILTER.VALUE.V18 date filter", async function () {
    mockDate("2022-03-10 00:00:00");
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=BETOPIAERP.FILTER.VALUE.V18("Date Filter")`);
    await animationFrame();
...
test("BETOPIAERP.FILTER.VALUE.V18 date from/to without values", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(model, "A1", `=BETOPIAERP.FILTER.VALUE.V18("Date Filter")`);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    expect(getEvaluatedCell(model, "B1").value).toBe(null);
});

test("BETOPIAERP.FILTER.VALUE.V18 date from/to with only from defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=BETOPIAERP.FILTER.VALUE.V18("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("BETOPIAERP.FILTER.VALUE.V18 date from/to with only to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=BETOPIAERP.FILTER.VALUE.V18("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("BETOPIAERP.FILTER.VALUE.V18 date from/to with from and to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=BETOPIAERP.FILTER.VALUE.V18("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("BETOPIAERP.FILTER.VALUE.V18 relation filter", async function () {
    const { model } = await createModelWithDataSource({
    ...
    setCellContent(model, "A10", `=BETOPIAERP.FILTER.VALUE.V18("Relation Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "relation",
...
test("BETOPIAERP.FILTER.VALUE.V18 with escaped quotes in the filter label", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "text",
    ...
    setCellContent(model, "A1", '=BETOPIAERP.FILTER.VALUE.V18("my \\"special\\" filter")');

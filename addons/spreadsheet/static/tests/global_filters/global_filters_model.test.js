...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    const chartId = model.getters.getBetopiaERPChartIds()[0];
    fieldMatching = model.getters.getBetopiaERPChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("field matching is removed when an BetopiaERP chart is deleted", async function () {
    ...
    expect(model.getters.getBetopiaERPChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", {
        figureId: model.getters.getFigureIdFromChartId(chartId),
        sheetId,
    });
    expect(() => model.getters.getBetopiaERPChartFieldMatching(chartId, filter.id)).toThrow(undefined, {

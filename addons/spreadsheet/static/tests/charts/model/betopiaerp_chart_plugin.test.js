...
import { BetopiaERPBarChart } from "@spreadsheet/chart/betopiaerp_chart/betopiaerp_bar_chart";
import { BetopiaERPChart } from "@spreadsheet/chart/betopiaerp_chart/betopiaerp_chart";
import { BetopiaERPLineChart } from "@spreadsheet/chart/betopiaerp_chart/betopiaerp_line_chart";
...
test("Can add an BetopiaERP Bar chart", async () => {
    ...
    expect(chart instanceof BetopiaERPBarChart).toBe(true);
...
test("Can add an BetopiaERP Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof BetopiaERPLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an BetopiaERP Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof BetopiaERPChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("BetopiaERP bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "betopiaerp_bar",
...
test("BetopiaERP pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "betopiaerp_pie",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("BetopiaERP line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "betopiaerp_line",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("Data reloaded upon domain update for charts other than pie/bar/line", async () => {
    ...
    updateChart(model, chartId, { type: "betopiaerp_pie" });
    await waitForDataLoaded(model);
...
test("Can import/export an BetopiaERP chart", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "betopiaerp_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("betopiaerp_line");
...
test("Can undo/redo an BetopiaERP chart creation", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "betopiaerp_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_pie" });
    insertChartInSpreadsheet(model, "betopiaerp_bar");
    insertChartInSpreadsheet(model, "betopiaerp_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste BetopiaERP chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof BetopiaERPChart).toBe(true);
...
test("Can cut/paste BetopiaERP chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof BetopiaERPChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates BetopiaERP chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load betopiaerp chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert betopiaerp chart from a different model", async () => {
    const { model } = await createModelWithDataSource();
...
test("BetopiaERP chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove betopiaerp chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getBetopiaERPChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getBetopiaERPChartIds().length).toBe(0);
});

test("BetopiaERP chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getBetopiaERPChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getBetopiaERPChartDisplayName(chartId)).toBe("(#1) BetopiaERP Line Chart");
});
...
test("BetopiaERP line and bar charts display only horizontal grid lines", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "betopiaerp_line",
    });
    const sheetId = model.getters.getActiveSheetId();
...
test("Can configure the chart datasets", async () => {
    ...
    const { model } = await createSpreadsheetWithChart({
        type: "betopiaerp_bar",
        serverData,
        definition: { type: "betopiaerp_bar", metaData, searchParams, id: "42" },
    });
    await waitForDataLoaded(model);
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("Non-web chart types are using data source in 'bar' mode", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "betopiaerp_radar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];

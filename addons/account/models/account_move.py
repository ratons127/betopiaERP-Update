...
from betopiaerp import api, fields, models, _, modules
from betopiaerp.tools.sql import column_exists, create_column
from betopiaerp.addons.account.tools import format_structured_reference_iso
from betopiaerp.exceptions import UserError, ValidationError, AccessError, RedirectWarning
...
from betopiaerp.tools.mail import email_re, email_split, is_html_empty, generate_tracking_message_id
from betopiaerp.tools.misc import StackMap
from betopiaerp.tools.safe_eval import safe_eval
...
class AccountMove(models.Model):
    ...
    quick_edit_total_amount = fields.Monetary(
        ...
        help='Use this field to encode the total amount of the invoice.\n'
             'BetopiaERP will automatically create one invoice line with default values to match it.',
    )
    ...
    def _get_fields_to_copy_recurring_entries(self, values):
        ...
        values.update({
            ...
            'invoice_user_id': self.invoice_user_id.id,  # otherwise user would be BetopiaERPBot
        })
    ...
    def _message_post_after_hook(self, new_message, message_values):
        ...
        if self.env.context.get('from_alias'):
            ...
            # Only trigger decoding if the message was sent by an active internal user (note BetopiaERPBot is always inactive).

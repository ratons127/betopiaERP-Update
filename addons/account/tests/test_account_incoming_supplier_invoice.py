...
from betopiaerp.tests import tagged, RecordCapturer
...
from betopiaerp.addons.test_mimetypes.tests.test_guess_mimetypes import contents
...
class TestAccountInvoiceImportMixin:
    ...
    def assert_attachment_import(self, origin, attachments_vals, expected_invoices):
        """ Simulate the upload and import of one or more attachments and assert that the
            ...
            :param attachments_vals: A list of values representing the attachments to be uploaded to BetopiaERP
...
class TestAccountIncomingSupplierInvoice(AccountTestInvoicingCommon, TestAccountInvoiceImportMixin, MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.internal_user = cls._create_new_internal_user(login='internal.user@test.betopiaerp.com')
    ...
    def test_import_with_traceback(self):
        ...
        with (
            ...
            patch('betopiaerp.addons.account.models.partner.ResPartner.search', side_effect=ValueError('We want to test an unexpected error')),
            mute_logger('betopiaerp.addons.account.models.account_document_import_mixin'),
        ):

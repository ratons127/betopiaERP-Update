...
from betopiaerp import api, fields, models, tools, SUPERUSER_ID, _
from betopiaerp.fields import Command, Date, Domain
from betopiaerp.addons.rating.models import rating_data
from betopiaerp.addons.html_editor.tools import handle_history_divergence
from betopiaerp.exceptions import UserError, ValidationError
from betopiaerp.tools import format_list, SQL, LazyTranslate, html_sanitize
from betopiaerp.addons.resource.models.utils import filter_domain_leaf
from betopiaerp.addons.project.controllers.project_sharing_chatter import ProjectSharingChatter
...
class ProjectTask(models.Model):
    ...
    # Need this field to check there is no email loops when BetopiaERP reply automatically
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-BetopiaERP-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-BetopiaERP-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-BetopiaERP-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
    ...
    def action_redirect_to_project_task_form(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': f"/betopiaerp/{self.project_id.id}/action-project.act_project_project_2_project_task_all/{self.id}?menu_id={menu_id}",
            'target': 'new',
        }

...
from betopiaerp.addons.project.tests.test_project_base import TestProjectCommon
...
class TestPortalProject(TestProjectPortalCommon):
    ...
    @mute_logger('betopiaerp.addons.base.ir.ir_model')
...
class TestAccessRightsPrivateTask(TestAccessRights):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.private_task = cls.env['project.task'].create({'name': 'BetopiaERPBot Private Task'})

...
from betopiaerp.addons.iot_drivers.controllers.proxy import proxy_drivers
from betopiaerp.addons.iot_drivers.iot_handlers.drivers.printer_driver_base import EscposNotAvailableError, PrinterDriverBase
from betopiaerp.addons.iot_drivers.main import iot_devices
from betopiaerp.addons.iot_drivers.tools import helpers, wifi, route
...
class PrinterDriver(PrinterDriverBase):
    ...
    def print_raw(self, data, action_unique_id=None):
        ...
        try:
            with self.cups_lock:
                job_id = self.conn.createJob(self.device_identifier, 'BetopiaERP print job', {'document-format': CUPS_FORMAT_AUTO})
                self.conn.startDocument(self.device_identifier, job_id, 'BetopiaERP print job', CUPS_FORMAT_AUTO, 1)

...
from betopiaerp.addons.iot_drivers.tools import certificate, helpers, route, upgrade, wifi
from betopiaerp.addons.iot_drivers.tools.system import IOT_SYSTEM, IS_RPI
from betopiaerp.addons.iot_drivers.main import iot_devices, unsupported_devices
...
class IotBoxOwlHomePage(http.Controller):
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "BetopiaERP's IoT Box - Handlers list",
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) BetopiaERP's configuration and logger to the given logger_name to the given level.
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get BetopiaERP Iot logger given an IoT handler name

...
from betopiaerp.addons.iot_drivers import main
from betopiaerp.addons.iot_drivers.tools import helpers, upgrade
from betopiaerp.addons.iot_drivers.server_logger import close_server_log_sender_handler
from betopiaerp.addons.iot_drivers.webrtc_client import webrtc_client
...
def send_to_controller(params, method="send_websocket", server_url=None):
    """Confirm the operation's completion by sending a response back to the BetopiaERP server

    :param params: the parameters to send back to the server
    :param method: method to call on the IoT box controller
    :param server_url: URL of the BetopiaERP server (provided by decorator).
    """
...
class WebsocketClient(Thread):
    ...
    def __init__(self, channel, server_url=None):
        ...
        :param str server_url: URL of the BetopiaERP server (provided by decorator).
        """
    ...
    def run(self):
        ...
        self.ws = websocket.WebSocketApp(self.websocket_url,
            header={"User-Agent": "BetopiaERPIoTBox/1.0", "Cookie": f"session_id={self.session_id}"},

...
from betopiaerp.addons.iot_drivers.tools.system import rpi_only, IS_RPI, IS_TEST
...
def get_db_branch(server_url):
    ...
    :param server_url: The URL of the connected BetopiaERP database.
    ...
    try:
        ...
        _logger.exception('Could not reach configured server to get the BetopiaERP version')
        return None
    try:
...
def check_version_upgrades(local_branch, db_branch):
    ...
    :param db_branch: The git branch of the connected BetopiaERP database (Ex: "saas-19.1" / "master" etc.)
    """
    try:
...
def check_git_branch(server_url=None):
    """Check if the local branch is the same as the connected BetopiaERP DB and
    ...
    try:
        ...
        _logger.info("IoT Box git branch: %s / Associated BetopiaERP db's git branch: %s", local_branch, db_branch)

...
from betopiaerp import http, release, service
from betopiaerp.addons.iot_drivers.tools.system import (
    IOT_CHAR,
    IOT_RPI_CHAR,
    IOT_WINDOWS_CHAR,
    IS_RPI,
...
from betopiaerp.tools.func import reset_cached_properties
...
def check_image():
    ...
    """
    try:
        response = requests.get('https://nightly.betopiaerp.com/master/iotbox/SHA1SUMS.txt', timeout=5)
...
def save_conf_server(url, token, db_uuid, enterprise_code, db_name=None):
    """
    Save server configurations in betopiaerp.conf
...
def download_iot_handlers(auto=True, server_url=None):
    """Get the drivers from the configured BetopiaERP server.
    ...
    :param server_url: The URL of the connected BetopiaERP database (provided by decorator).
    """
...
def compute_iot_handlers_addon_name(handler_kind, handler_file_name):
    return "betopiaerp.addons.iot_drivers.iot_handlers.{handler_kind}.{handler_name}".\
...
def path_file(*args):
    """Return the path to the file from IoT Box root or Windows BetopiaERP
    server folder
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\BetopiaERP\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\BetopiaERP\\downloaded_file.zip', 'C:\\Program Files\\BetopiaERP\\new_folder'))
...
def update_conf(values, section='iot.box'):
    """Update betopiaerp.conf with the given key and value.
    ...
    """
    _logger.debug("Updating betopiaerp.conf with values: %s", values)
    conf = get_conf()

    if not conf.has_section(section):
        _logger.debug("Creating new section '%s' in betopiaerp.conf", section)
        conf.add_section(section)

    for key, value in values.items():
        conf.set(section, key, value) if value else conf.remove_option(section, key)

        with open(path_file("betopiaerp.conf"), "w", encoding='utf-8') as f:
            conf.write(f)


def get_conf(key=None, section='iot.box'):
    """Get the value of the given key from betopiaerp.conf, or the full config if no key is provided.
    ...
    conf.read(path_file("betopiaerp.conf"))

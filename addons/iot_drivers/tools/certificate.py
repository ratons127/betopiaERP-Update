...
from betopiaerp.addons.iot_drivers.tools.system import IS_RPI, IS_TEST, IS_WINDOWS
...
def get_certificate_end_date():
    ...
    if (
        common_name == 'BetopiaERPTempIoTBoxCertificate'
...
def download_betopiaerp_certificate(retry=0):
    ...
    if server_error:
        _logger.error("Server error received from betopiaerp.com while trying to get the certificate: %s", server_error)
        return None

    result = response_body.get('result', {})
    certificate_error = result.get('error')
    if certificate_error:
        _logger.warning("Error received from betopiaerp.com while trying to get the certificate: %s", certificate_error)
        return None
    ...
    if not certificate or not private_key:  # ensure not empty strings
        _logger.error("The certificate received from betopiaerp.com is not valid.")
        return None

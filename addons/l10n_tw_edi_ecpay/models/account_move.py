...
from betopiaerp.addons.l10n_tw_edi_ecpay.utils import call_ecpay_api, transfer_time, convert_utc_time_to_tw_time
from betopiaerp.exceptions import UserError
from betopiaerp.tools import float_round
...
class AccountMove(models.Model):
    ...
    def _l10n_tw_edi_send(self, json_content):
        """
        Issuing an e-invoice by calling the Ecpay API and update the invoicing result in BetopiaERP
        """
        self.ensure_one()
    ...
    def _l10n_tw_edi_update_ecpay_invoice_info(self):
        """
        Searching the e-invoice information from Ecpay API and update the invoice information in BetopiaERP
        """
        self.ensure_one()
        # Ensure to lock the records that will be sent, to avoid risking sending them twice.
        self.env["res.company"]._with_locked_records(self)
    ...
    def _l10n_tw_edi_run_invoice_invalid(self):
        """
        Cancelling the e-invoice by calling the Ecpay API and update the invoice information in BetopiaERP
        """
        self.ensure_one()
        ...
        # update the invoice information in BetopiaERP
    ...
    def _l10n_tw_edi_issue_allowance(self, json_content):
        """
        Issuing an allowance by calling the Ecpay API and update the refund invoice information in BetopiaERP

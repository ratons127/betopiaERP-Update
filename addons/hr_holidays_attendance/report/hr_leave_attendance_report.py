...
from betopiaerp.tools.sql import SQL, drop_view_if_exists
...
class HrLeaveAttendanceReport(models.Model):
        ...
        return """
            LEFT JOIN (
                  ...
                  AND CAST(rca.dayofweek AS INTEGER) = (
                        ...
                        END
                      )  -- to map days between BetopiaERP (Monday = 0, Tuesday = 1, ...) and Postgres (Sunday = 0, Monday = 1, ...)
        """
    ...
    def _join_daily_leave_hours(self):
        """ Generates a SQL join clause to calculate the total `leave_hours` taken for a specific day:
            ...
            - Converts all date fields to UTC to maintain consistency with BetopiaERP's date storage.
        """
        return """
            LEFT JOIN LATERAL (

...
from betopiaerp import SUPERUSER_ID, api, fields, models, modules
from betopiaerp.exceptions import UserError
from betopiaerp.fields import Domain
from betopiaerp.tools import config, date_utils, split_every
...
class MyInvoisDocument(models.Model):
    ...
    In BetopiaERP, a document represent either an invoice, or a group of PoS order.
    """
    ...
    # BetopiaERP Implementation fields
    ...
    def _myinvois_map_error(self, error):
        ...
        error_map = {
            ...
            "internal_server_error": self.env._(
                "Server error; If the problem persists, please contact the BetopiaERP support."
            ),
            # The proxy user credentials are either incorrect, or BetopiaERP does not have the permission to invoice on their behalf.
            "invalid_tin": self.env._(
                "Please make sure that your company TIN is correct, and that you gave BetopiaERP sufficient permissions on the MyInvois platform."
            ),
            ...
            'action_forbidden': self.env._('Permission to do this action has not been granted. Please ensure that BetopiaERP has sufficient permissions on the MyInvois platform.'),
        }
    ...
    def _myinvois_set_state(self, state, message=None):
        ...
        # In BetopiaERP, the best way to represent that is by cancelling the invoice.
    ...
    def _myinvois_set_validation_fields(self, validation_result):
        ...
        # BetopiaERP expect a timezone unaware datetime in UTC, so we can safely remove the info without any more work needed.

...
from betopiaerp.addons.certificate.tools import CertificateAdapter
from betopiaerp.exceptions import UserError
from betopiaerp.tools import float_repr, float_round, frozendict, zeep
...
class L10nEsEdiVerifactuDocument(models.Model):
    ...
    def _check_record_values(self, vals):
        ...
        # We currently do not support cancelling records that are not registered or were registered outside betopiaerp.
        if vals['cancellation'] and not verifactu_registered:
            errors.append(_("The cancelled record is not Veri*Factu registered (inside BetopiaERP)."))
    ...
    def _render_vals_SistemaInformatico(self, vals):
        ...
        render_vals = {
            'SistemaInformatico': {
                ...
                'NombreSistemaInformatico': 'BetopiaERP',
                'IdSistemaInformatico': '00',  # identifies BetopiaERP the software as product of BetopiaERP the company

import { expect, test, waitFor } from "@betopiaerp/hoot";
import {
    contains,
    getService,
    mockService,
...
test("handles BetopiaERP's jsonrpc error", async () => {
    await mountWithCleanup(Parent);
    onRpc("/test/", () => ({
        error: {
            data: {
                name: "ValidationError",
                message: "Boom BetopiaERP",
            },
        },
    }));
    const fileUploadService = await getService("file_upload");
    fileUploadService.upload("/test/", []);
    await waitFor(".o_notification:has(.bg-danger):contains(ValidationError: Boom BetopiaERP)");
});

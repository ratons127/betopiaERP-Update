...
function BetopiaERPEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
BetopiaERPEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
BetopiaERPEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * BetopiaERP where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new BetopiaERPEvent(this, name, info);

...
import betopiaerp
import betopiaerp.modules.registry
from betopiaerp import http
from betopiaerp.http import content_disposition, dispatch_rpc, request, Response
from betopiaerp.service import db
from betopiaerp.tools.misc import file_open, str2bool
from betopiaerp.tools.translate import _

from betopiaerp.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = betopiaerp.tools.config.verify_admin_password('admin')
        d['list_db'] = betopiaerp.tools.config['list_db']
        d['langs'] = betopiaerp.service.db.exp_list_lang()
        d['countries'] = betopiaerp.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = betopiaerp.service.db.list_db_incompatible(d['databases'])
        except betopiaerp.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = betopiaerp.tools.config.verify_admin_password('admin')
        ...
        try:
            ...
            with betopiaerp.modules.registry.Registry(name).cursor() as cr:
    ...
    def backup(self, master_pwd, name, backup_format='zip', filestore=True):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            betopiaerp.service.db.check_super(master_pwd)
            ...
            dump_stream = betopiaerp.service.db.dump_db(name, None, backup_format, filestore)

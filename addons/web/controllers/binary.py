...
try:
    ...
    from betopiaerp.tools._vendor.send_file import send_file

import betopiaerp
import betopiaerp.modules.registry
from betopiaerp import SUPERUSER_ID, _, http, api
from betopiaerp.addons.base.models.assetsbundle import ANY_UNIQUE
from betopiaerp.exceptions import AccessError, UserError
from betopiaerp.http import request, Response
from betopiaerp.tools import file_open, file_path, replace_exceptions, str2bool
from betopiaerp.tools.image import image_guess_size_from_field_name
from betopiaerp.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
BetopiaERP is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however BetopiaERP is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if betopiaerp.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=betopiaerp.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or betopiaerp.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the BetopiaERP logo instead", exc_info=True)

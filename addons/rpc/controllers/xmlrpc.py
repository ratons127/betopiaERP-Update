...
import betopiaerp.exceptions
from betopiaerp.fields import Command, Date, Datetime
from betopiaerp.http import Controller, Response, dispatch_rpc, request, route
from betopiaerp.tools import lazy
from betopiaerp.tools.misc import frozendict
...
# User code must use the exceptions defined in ``betopiaerp.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, betopiaerp.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, betopiaerp.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, betopiaerp.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, betopiaerp.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, betopiaerp.exceptions.MissingError):
    ...
    # InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(betopiaerp.tools.exception_to_unicode(e), formatted_info)

    return dumps(fault)


class BetopiaERPMarshaller(xmlrpc.client.Marshaller):
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = BetopiaERPMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}

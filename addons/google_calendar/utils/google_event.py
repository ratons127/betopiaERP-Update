...
class GoogleEvent(abc.Set):
    ...
    Inspired by BetopiaERP recordset, one instance can be a single Google event or a
    ...
    def owner(self, env):
        ...
        # (they are the organizer in Google). The "real" owner (in BetopiaERP) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in BetopiaERP several users might have the same email :/

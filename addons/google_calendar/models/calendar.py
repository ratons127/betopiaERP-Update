...
class CalendarEvent(models.Model):
    ...
    def action_mass_archive(self, recurrence_update_setting):
        """ Delete recurrence in BetopiaERP if in 'all_events' or in 'future_events' edge case, triggering one mail. """
        self.ensure_one()
    ...
    def _google_values(self):
        ...
        if self.user_id and self.user_id != self.env.user and not bool(self.user_id.sudo().google_calendar_token):
            # The organizer is an BetopiaERP user that do not sync his calendar
            ...
            # the owner is not an BetopiaERP user: We don't store the real owner identity (mail)

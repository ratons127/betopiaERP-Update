...
from betopiaerp.tests import HttpCase, new_test_user, tagged, TransactionCase
from betopiaerp.tests.common import HOST

from betopiaerp.addons.auth_totp.models.totp import TOTP
from betopiaerp.addons.auth_totp.controllers.home import TRUSTED_DEVICE_COOKIE
...
class TestAuthTimeoutHttp(HttpCase):
    ...
    def set_session_create_time(self, session_id, timestamp):
        session = betopiaerp.http.root.session_store.get(session_id)
    ...
    def assertMustCheckIdentity(self):
        ...
        self.assertEqual(
            ...
            "betopiaerp.addons.auth_timeout.models.ir_http.CheckIdentityException",
        )

    def assertSessionExpired(self):
        result = self.rpc("res.users", "read", [self.user.id], ["login"])
        self.assertEqual(result.get("error", {}).get("data", {}).get("name"), "betopiaerp.http.SessionExpiredException")

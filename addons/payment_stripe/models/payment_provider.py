...
from betopiaerp.exceptions import RedirectWarning, UserError, ValidationError
from betopiaerp.tools.urls import urljoin as url_join

from betopiaerp.addons.payment import utils as payment_utils
from betopiaerp.addons.payment.logging import get_payment_logger
from betopiaerp.addons.payment_stripe import const
from betopiaerp.addons.payment_stripe import utils as stripe_utils
from betopiaerp.addons.payment_stripe.controllers.main import StripeController
from betopiaerp.addons.payment_stripe.controllers.onboarding import OnboardingController
...
class PaymentProvider(models.Model):
    ...
    stripe_webhook_secret = fields.Char(
        ...
        help="If a webhook is enabled on your Stripe account, this signing secret must be set to "
             "authenticate the messages sent from Stripe to BetopiaERP.",
        copy=False,
        groups='base.group_system',
    )
    ...
    def action_start_onboarding(self, menu_id=None):
        ...
        the URL the user is redirected to when coming back on BetopiaERP after the onboarding. If the link

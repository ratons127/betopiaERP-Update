...
from betopiaerp.addons.payment.controllers.portal import PaymentPortal
...
class TestFlows(PaymentHttpCommon):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger("betopiaerp.http"), self.assertRaises(
            JsonRpcException,
            msg='betopiaerp.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.',
        ):
    ...
    def test_transaction_wrong_token(self):
        ...
        with mute_logger('betopiaerp.http'), self.assertRaises(JsonRpcException, msg='betopiaerp.exceptions.ValidationError: The access token is invalid.'):
    ...
    @mute_logger('betopiaerp.addons.payment.models.payment_transaction')
    def test_direct_payment_triggers_no_payment_request(self):
        self.authenticate(self.portal_user.login, self.portal_user.login)
        self.partner = self.portal_partner
        self.user = self.portal_user
        with patch(
            'betopiaerp.addons.payment.models.payment_transaction.PaymentTransaction'
            '._charge_with_token'
        ) as patched:

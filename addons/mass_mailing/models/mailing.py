...
from betopiaerp.addons.base_import.models.base_import import ImportValidationError
from betopiaerp.exceptions import UserError, ValidationError
from betopiaerp.fields import Datetime, Domain
from betopiaerp.tools.float_utils import float_round
from betopiaerp.tools.image import ImageProcess
...
class MailingMailing(models.Model):
    ...
    mail_server_id = fields.Many2one('ir.mail_server', string='Mail Server',
        ...
        help="Use a specific mail server in priority. Otherwise BetopiaERP relies on the first outgoing mail server available (based on their sequencing) as it does for normal mails.")
    ...
    def _action_send_mail(self, res_ids=None):
        ...
        for mailing in self:
            ...
            composer_values = {
                ...
                # If current user is betopiaerpbot, use mailing responsible (no impact on email_from)
                'author_id': mailing.user_id.partner_id.id if user_partner == betopiaerpbot else user_partner.id,
    ...
    def _prepare_statistics_email_values(self):
        ...
        return {
            ...
            'top_button_url': tools.urls.urljoin(web_base_url, f'/betopiaerp/mailing.mailing/{self.id}'),
            'kpi_data': [
                kpi,
                {

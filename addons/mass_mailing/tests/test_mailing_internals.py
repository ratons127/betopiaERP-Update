...
from betopiaerp.addons.mass_mailing.tests.common import MassMailCommon
from betopiaerp.exceptions import ValidationError
from betopiaerp.sql_db import Cursor
from betopiaerp.tests import Form, HttpCase, users, tagged
...
class TestMassMailValues(MassMailCommon):
    ...
    def test_mailing_body_cropped_vml_image(self):
        ...
        def patched_images_to_urls(self, b64images):
            ...
            if len(urls) == 1:
                ...
                return []
        with patch("betopiaerp.addons.mass_mailing.models.mailing.MailingMailing._get_image_by_url",
                   new=patched_get_image), \
             patch("betopiaerp.addons.mass_mailing.models.mailing.MailingMailing._create_attachments_from_inline_images",
    ...
    def test_mailing_body_inline_image(self):
        ...
        def patched_images_to_urls(self, b64images):
            ...
            return urls
        with patch("betopiaerp.addons.mass_mailing.models.mailing.MailingMailing._create_attachments_from_inline_images",
                   new=patched_images_to_urls):
            mailing = self.env['mailing.mailing'].create({
                    'name': 'Test',
                    'subject': 'Test',
    ...
    def test_mailing_computed_fields_domain_w_filter(self):
        ...
        filter_1, filter_2, filter_3 = self.env['mailing.filter'].create([
            ...
            {'name': 'Email based',
             'mailing_domain' : [('email', 'ilike', 'info@betopiaerp.com')],
             'mailing_model_id': self.env['ir.model']._get('res.partner').id,
            }
        ])
        ...
        mailing.mailing_domain = "[('email', 'ilike', 'info_be@betopiaerp.com')]"
...
Email: <a id="url5" href="mailto:test@betopiaerp.com">test@betopiaerp.com</a></div>""",
            'mailing_model_id': self.env['ir.model']._get('mailing.list').id,
            'reply_to_mode': 'new',
            'reply_to': self.email_reply_to,
            'contact_list_ids': [(6, 0, self.mailing_list_1.ids)],

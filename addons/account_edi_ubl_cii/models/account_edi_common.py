...
from betopiaerp import _, api, models
from betopiaerp.addons.base.models.res_bank import sanitize_account_number
from betopiaerp.exceptions import UserError, ValidationError
from betopiaerp.tools import float_is_zero, float_repr
from betopiaerp.tools.float_utils import float_round
from betopiaerp.tools.misc import clean_context, formatLang, html_escape
from betopiaerp.tools.xml_utils import find_xml_value
...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_invoice_ubl_cii(self, invoice, file_data, new=False):
        ...
        # This has to be done after the first import in order to let BetopiaERP compute the taxes before overriding if needed.
        with invoice._get_edi_creation() as invoice:
    ...
    def _retrieve_taxes(self, record, line_values, tax_type, tax_exigibility=False):
        ...
        In a UBL/CII xml, the BetopiaERP "price_include" concept does not exist. Hence, first look for a price_include=False,
    ...
    def _retrieve_line_charges(self, record, line_values, taxes):
        ...
        Special case: if the ReasonCode == 'AEO', there is a high chance the xml was produced by BetopiaERP and the

...
from betopiaerp.tests import Form, users, new_test_user, HttpCase, tagged, TransactionCase
from betopiaerp.addons.hr.tests.common import TestHrCommon
...
class TestHrEmployee(TestHrCommon):
    ...
    def test_employee_timezone(self):
        ...
        with mute_logger('betopiaerp.sql_db'), self.assertRaises(NotNullViolation):
            employee.tz = False
        ...
        user_1, user_2, user_3 = self.env['res.users'].create([
            {
                'name': 'Test User',
                'login': 'test_user',
                'email': 'test_user@betopiaerp.com',
            },
            {
                'name': 'Test User 2',
                'login': 'test_user_2',
                'email': 'test_user_2@betopiaerp.com',
                'create_employee': True,
            },
            {
                'name': 'Test User 3',
                'login': 'test_user_3',
                'email': 'test_user_3@betopiaerp.com',
    ...
    def test_set_user_on_new_employee(self):
        ...
        employee_form.user_id = self.res_users_hr_officer
        with mute_logger('betopiaerp.sql_db'), self.assertRaises(UniqueViolation), self.assertRaises(ValidationError):
            employee_form.save()
    ...
    def test_search_hr_employee_no_access(self):
        ...
        with self.assertNoLogs('betopiaerp.domains'):
            self.assertEqual(
                employee.ids,
...
class TestHrEmployeeLinks(HttpCase):
    def test_shared_private_link_permissions(self):
        ...
        with mute_logger('betopiaerp.http'):  # ignore raised RedirectWarning
            self.start_tour(
                f"/betopiaerp/employees/{employee_sonic.id}",

...
from betopiaerp.tools import float_round, float_repr
...
class AccountMove(models.Model):
    ...
    def action_l10n_vn_edi_update_payment_status(self):
        ...
        for invoice in invoices:
            ...
            # SInvoice will return a NOT_FOUND_DATA error if the status in BetopiaERP matches the one on their side.
        ...
        try:
            ...
            self.message_post(
                body=_('The invoice has been canceled on sinvoice for reason: %(reason)s'
                       'But the cancellation in BetopiaERP failed with error: %(error)s', reason=reason, error=e),
            )

    ...
    def _make_request(self, url, params=False, *, auth_type: Literal['hmac', 'asymmetric'] = 'hmac'):
        ...
        try:
            res = requests.post(
                ...
                auth=BetopiaERPEdiProxyAuth(user=self, auth_type=auth_type))
        ...
        if proxy_error:
            ...
            if error_code == 'invalid_signature':
                raise AccountEdiProxyError(
                    error_code,
                    _("Failed to connect to BetopiaERP Access Point server. This might be due to another connection to BetopiaERP Access Point "
        ...
        if edi_mode == 'demo':
            ...
            if 'error' in response:
                if response['error'] == 'A user already exists with this identification.':
                    ...
                    # BetopiaERP side. We are for now forced to check the error message.

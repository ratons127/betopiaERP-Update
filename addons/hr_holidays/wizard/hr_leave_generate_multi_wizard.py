...
class HrLeaveGenerateMultiWizard(models.TransientModel):
    ...
    def action_generate_time_off(self):
        ...
        if conflicting_leaves:
            ...
            if invalid_time_off:
                raise UserError(self.env._('Some employees already have time off requests in hours that overlap with the selected period, BetopiaERP cannot automatically adjust or split hourly leaves during batch generation. Conflicting time off:\n%s', '\n'.join(f"- {l.display_name}" for l in invalid_time_off)))

...
from betopiaerp import http, models, fields, tools, _
from betopiaerp.exceptions import AccessError, UserError
from betopiaerp.fields import Domain
from betopiaerp.http import request, SessionExpiredException
from betopiaerp.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from betopiaerp.tools.translate import LazyTranslate
from betopiaerp.addons.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
from betopiaerp.addons.portal.controllers.portal import pager as portal_pager
from betopiaerp.addons.portal.controllers.web import Home
from betopiaerp.addons.web.controllers.binary import Binary
from betopiaerp.addons.web.controllers.session import Session
from betopiaerp.addons.website.tools import get_base_domain
from betopiaerp.tools.json import scriptsafe as json
from betopiaerp.tools.translate import TRANSLATED_ELEMENTS
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/betopiaerp?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_info(self, **kwargs):
        ...
        values = {
            'apps': apps,
            'l10n': l10n,
            'version': betopiaerp.service.common.exp_version()
        }
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/betopiaerp/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/betopiaerp/ir.ui.view/{page.get('view_id')}")

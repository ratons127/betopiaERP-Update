...
class TestWebsiteAssets(betopiaerp.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/betopiaerp')
        check_asset()
        self.url_open(domain_2 + '/betopiaerp')
        check_asset()
        self.url_open(domain_1 + '/betopiaerp')
        check_asset()
        self.url_open(domain_2 + '/betopiaerp')
...
class TestWebAssets(betopiaerp.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with betopiaerp.tools.mute_logger('betopiaerp.addons.web.controllers.binary'):
            self.assertEqual(

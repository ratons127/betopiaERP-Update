...
from betopiaerp.tools.misc import ReadonlyDict


class MicrosoftEvent(abc.Set):
    """
    This helper class holds the values of a Microsoft event.
    Inspired by BetopiaERP recordset, one instance can be a single Microsoft event or a
    ...
    def _load_betopiaerp_ids_from_db(self, env, force_model=None):
        ...
        2) match BetopiaERP events and Outlook events which have both a ICalUId set,
    ...
    def owner_id(self, env):
        ...
        1) the current BetopiaERP user is the organizer of the event according to Outlook event, so return his id.
        2) the current BetopiaERP user is NOT the organizer and:
           2.1) we are able to find a BetopiaERP user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a BetopiaERP user matching the organizer email address and we return False, meaning
                that no BetopiaERP user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        ...
        if organizer_email:
            # Warning: In Microsoft: 1 email = 1 user; but in BetopiaERP several users might have the same email

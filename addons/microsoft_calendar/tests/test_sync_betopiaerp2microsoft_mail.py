...
from betopiaerp.addons.microsoft_calendar.tests.common import TestCommon


class TestSyncBetopiaERP2MicrosoftMail(TestCommon, MailCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
    ...
    def test_change_organizer_pure_betopiaerp_event(self):
        """
        Test that changing organizer on a pure BetopiaERP event (not synced with Microsoft)
        ...
        self.assertTrue(event.active, "Pure BetopiaERP event should not be archived when changing organizer")

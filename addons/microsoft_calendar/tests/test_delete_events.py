...
from betopiaerp.addons.microsoft_calendar.tests.common import (
    TestCommon,
    mock_get_token,
...
class TestDeleteEvents(TestCommon):
    ...
    def test_delete_simple_event_from_outlook_attendee_calendar(self):
        """
        If an attendee deletes an event from its Outlook calendar, during the sync, BetopiaERP will be notified that
        this event has been deleted BUT only with the attendees's calendar event id and not with the global one
        (called iCalUId). That means, it's not possible to match this deleted event with an BetopiaERP event.

        LIMITATION:

        Unfortunately, there is no magic solution:
            1) keep the list of calendar events ids linked to a unique iCalUId but all BetopiaERP users may not have synced
            their BetopiaERP calendar, leading to missing ids in the list => bad solution.
    ...
    def test_delete_synced_event_with_sync_config_paused(self, mock_delete):
        ...
        # Try to delete a simple event in BetopiaERP Calendar.
        self.simple_event.with_user(self.organizer_user).unlink()
        self.call_post_commit_hooks()
        self.simple_event.invalidate_recordset()

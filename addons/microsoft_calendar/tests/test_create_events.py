...
from betopiaerp import Command, fields

from betopiaerp.addons.mail.tests.common import mail_new_test_user
from betopiaerp.addons.microsoft_calendar.utils.microsoft_calendar import MicrosoftCalendarService
from betopiaerp.addons.microsoft_calendar.utils.microsoft_event import MicrosoftEvent
from betopiaerp.addons.microsoft_calendar.models.res_users import ResUsers
from betopiaerp.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token, _modified_date_in_the_future
from betopiaerp.exceptions import ValidationError, UserError
from betopiaerp.tests.common import tagged
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A BetopiaERP event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_simple_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the BetopiaERP organizer calendar.
        """
    ...
    def test_create_recurrent_event_with_sync_by_another_user(self, mock_insert, mock_get_events):
        """
        A BetopiaERP recurrent event has been created and synced with Outlook by another user, but nothing
    ...
    def test_create_recurrent_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the BetopiaERP organizer calendar.
        """
    ...
    def test_create_recurrent_event_from_outlook_attendee_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the BetopiaERP attendee calendar.
        """
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in BetopiaERP due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in BetopiaERP Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its BetopiaERP Calendar synced.
        ...
        # In BetopiaERP, this event will be created (behind the screens) by a synced BetopiaERP user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with BetopiaERP through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in BetopiaERP (no ACL error was triggered blocking creation).
    ...
    def test_create_event_with_default_and_undefined_sensitivity(self):
        ...
        # Sync events from Microsoft to BetopiaERP after adding the sensitivity (privacy) property.
    ...
    def test_create_videocall_sync_microsoft_calendar(self, mock_insert, mock_get_events):
        ...
        # Verify the event's videocall_location is updated in BetopiaERP
    ...
    def test_new_db_skip_betopiaerp2microsoft_sync_previously_created_events(self, mock_insert, mock_get_events):
        ...
        # Ten seconds later the ICP parameter saving, make the synchronization between BetopiaERP
        ...
        with self.mock_datetime_and_now('2024-01-02 10:00:10'):
            # Mock the return of 0 events from Outlook to BetopiaERP, then activate the user's sync.
            mock_get_events.return_value = ([], None)
            self.organizer_user.microsoft_synchronization_stopped = False
            self.organizer_user.microsoft_calendar_token_validity = datetime.now() + timedelta(minutes=60)
            self.assertTrue(self.env['calendar.event'].with_user(self.organizer_user)._check_microsoft_sync_status())

...
from betopiaerp import Command

from betopiaerp.addons.microsoft_calendar.models.microsoft_sync import MicrosoftCalendarSync
...
from betopiaerp.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token, _modified_date_in_the_future, patch_api
from betopiaerp.exceptions import UserError, ValidationError
...
class TestUpdateEvents(TestCommon):
    ...
    # -------------------------------------------------------------------------------
    # Update from Outlook to BetopiaERP
    # -------------------------------------------------------------------------------
    ...
    def test_forbid_simple_event_become_recurrence_sync_on(self, mock_patch):
        """
        Forbid in BetopiaERP simple event becoming a recurrence when Outlook Calendar sync is active.
        """
    ...
    def test_update_synced_event_with_sync_config_paused(self, mock_patch):
        ...
        # Try to update a simple event in BetopiaERP Calendar.
    ...
    def test_changing_event_organizer_to_another_user(self, mock_insert, mock_delete, mock_get_events):
        """
        Allow editing the event organizer to another user only if the proposed organizer have its BetopiaERP Calendar synced.
    ...
    def test_restart_sync_with_synced_recurrence(self, mock_patch):
        """ Ensure that sync restart is not blocked when there are recurrence outliers in BetopiaERP database. """
    ...
    def test_update_old_event_synced_with_outlook(self, mock_get_events, mock_write_from_microsoft):
        """
        There are old events in BetopiaERP which share the same state with Microsoft and get updated (without changes) in BetopiaERP
        due to a few seconds of update time difference, triggering lots of unwanted spam for attendees on Microsoft side.
        Don't update old events in BetopiaERP if update time difference between Microsoft and BetopiaERP is not significant.
        """

...
from betopiaerp.addons.microsoft_account.models.microsoft_service import TIMEOUT
...
class MicrosoftCalendarSync(models.AbstractModel):
    ...
    def _update_microsoft_recurrence(self, recurrence, events):
        """
        Update BetopiaERP events from Outlook recurrence and events.
        """
    ...
    def _check_old_event_update_required(self, lower_bound_day_range, update_time_diff):
        """
        Checks if an old event in BetopiaERP should be updated locally. This verification is necessary because
        sometimes events in BetopiaERP have the same state in Microsoft and even so they trigger updates locally
    ...
    def _microsoft_delete(self, user_id, event_id, timeout=TIMEOUT):
        """
        Once the event has been really removed from the BetopiaERP database, remove it from the Outlook calendar.
    ...
    def _microsoft_patch(self, user_id, event_id, values, timeout=TIMEOUT):
        """
        Once the event has been really modified in the BetopiaERP database, modify it in the Outlook calendar.
    ...
    def _microsoft_insert(self, values, timeout=TIMEOUT):
        """
        Once the event has been really added in the BetopiaERP database, add it in the Outlook calendar.
    ...
    def _microsoft_attendee_answer(self, answer, params, timeout=TIMEOUT):
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                # former differs for each attendee. This info is required for sending the event answer and BetopiaERP currently
    ...
    def _is_microsoft_insertion_blocked(self, sender_user):
        ...
        This is a necessary step for ensuring data match between BetopiaERP and Microsoft,

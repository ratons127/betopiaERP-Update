...
from betopiaerp.addons.website.models.website import Website
...
class TestWebsiteSession(HttpCaseWithUserDemo):
    ...
    def test_02_inactive_session_lang(self):
        ...
        current_website_id = self.env["website"]._get_current_website_id(betopiaerp.tests.HOST)
        self.env["website"].browse(current_website_id).domain = betopiaerp.tests.HOST
    ...
    def test_branding_cache(self):
        ...
        # Public user.
        self.opener.cookies.set("session_id", public_session.sid, domain=betopiaerp.tests.common.HOST)
        result = self.url_open(f'/test_website/model_item_sudo/{record.id}')
        self.assertFalse(has_branding(result.text), "Should have no branding for public user")

        # Back to demo user.
        self.opener.cookies.set("session_id", demo_session.sid, domain=betopiaerp.tests.common.HOST)
        result = self.url_open(f'/test_website/model_item_sudo/{record.id}')
        self.assertTrue(has_branding(result.text), "Should have branding for user demo")

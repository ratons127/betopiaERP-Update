...
from betopiaerp.exceptions import AccessDenied, AccessError, UserError
from betopiaerp.fields import Domain
from betopiaerp.http import request
from betopiaerp.modules.module import MANIFEST_NAMES, Manifest
from betopiaerp.release import major_version
from betopiaerp.tools import SQL, convert_file
from betopiaerp.tools import file_open, file_path, file_open_temporary_directory, ormcache
from betopiaerp.tools.misc import OrderedSet, topological_sort
from betopiaerp.tools.translate import JAVASCRIPT_TRANSLATION_COMMENT, CodeTranslations, TranslationImporter, get_base_langs

from betopiaerp.addons.base.models.ir_asset import is_wildcard_glob

_logger = logging.getLogger(__name__)

APPS_URL = "https://apps.betopiaerp.com"
...
class IrModuleModule(models.Model):
    ...
    def _import_module(self, module, path, force=False, with_demo=False):
        ...
        if unmet_dependencies:
            ...
            raise UserError(_("Studio customizations require the BetopiaERP Studio app."))
    ...
    def _extract_resource_attachment_translations(self, module, lang):
        ...
        for attachment in attachments.filtered('raw'):
            ...
            if attachment.url.endswith('js'):
                extract_method = 'betopiaerp.tools.babel:extract_javascript'
                extract_keywords = {'_t': None}
            else:
                extract_method = 'betopiaerp.tools.translate:babel_extract_qweb'
                extract_keywords = {}
            try:
                with io.BytesIO(attachment.raw) as fileobj:
